...
//    def hashRate(i: Int, prev: HashRate): BigInt = if (i % 2 == 1) R else R + Ra
  ...
  val res = simulate(1, Seq((0, BigInt(1000), BigInt(1000), desired)), 1000).reverse
  val file = new File("data/Bitcoin/random.txt")
...
class LinearDifficultyControl extends DifficultyControl {
  override def diff(lastDiffs: Seq[(Difficulty, FiniteDuration)], desired: FiniteDuration): BigInt = {
    if (lastDiffs.size > 2) {
      val realDiffs: Seq[BigInt] = lastDiffs.map(l => l._1 * desired.toMillis / l._2.toMillis).toSeq
      val data: Seq[(Int, Difficulty)] = (1 to realDiffs.size).map(i => i -> realDiffs(i))
      interpolate(data)(data.map(_._1).max + 1)
    } else lastDiffs.head._1
  }

  //y = a + bx
  private def interpolate(data: Seq[(Int, BigInt)]): (Int) => BigInt = {
